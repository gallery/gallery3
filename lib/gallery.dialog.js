(function($) {
  $.widget("ui.gallery_dialog", {
    ...
    _layout: function() {
      // Reset the dialog width before setting it (allows dialogs to resize for new content)
      $("#g-dialog").dialog("option", "width", false);
    ...
    form_loaded: function(event, ui) {
      ...
      // Initialize dialog links inside the dialog - allows dialogs to be replaced by new dialogs
      $("#g-dialog .g-dialog-link").gallery_dialog();
    },
      ...
      $("#g-dialog form").ajaxForm({
        ...
        success: function(data) {
          ...
          if (data.result == "success") {
            if (data.location) {
              if (data.show_in_dialog) {
                self._show(data.location);
              } else {
