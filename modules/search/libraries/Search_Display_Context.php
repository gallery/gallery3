...
class Search_Display_Context_Core extends Display_Context {
  ...
  function display_context($item) {
    $position = $this->get_position($item);
  ...
  function bread_crumb($item) {
    ...
    return array(item::root(), $this->dynamic_item($this->get("title"),
                                                      "search?q=" . urlencode($this->get("q")) . "?show={$item->id}"));
  }
   ...
   */
  private function get_position($item, $where=array()) {
    list($count, $data) = search::search($this->get("query_terms"), $this->get("page_size"),
                                       $this->get("offset"));
    ...
    return  $this->get("offset") + $page_position;
  }
}
