...
class gallery_installer {
  static function install() {
    ...
    module::set_var("gallery", "convert_photo_resize", "");
    module::set_var("gallery", "convert_photo_thumb", "");
    module::set_var("gallery", "convert_movie_thumb", "jpg");
  ...
  static function upgrade($version) {
    ...
    if ($version == 53) {
      // In v54, we add the independent definition of the extension of resize and thumb images.  
      // Previously, those of photos were set to follow the extension of the full-size image,
      // and album and movie thumbnails were set as jpg.  Among other issues, this meant that
      // album covers taken from non-jpg photos and copies of missing_movie and missing_photo could
      // have incorrect extensions.  The changes in this version ensure that this will not occur in
      // the future, and the code below corrects album thumbnails (but does not attempt to fix
      // copies of missing_movie and missing_photo).  Also, by defining these independently,
      // we can change them (e.g. to make all thumbnails into jpg for fast page loads).  Finally,
      // we're setting resize_dirty to 0 for movies and albums, as they were not consistently left
      // as 0 or 1 before.
      
      // Add two new columns to the items table.
      $db->query("ALTER TABLE {items} ADD COLUMN `resize_extension` varchar(6) DEFAULT NULL");
      $db->query("ALTER TABLE {items} ADD COLUMN `thumb_extension` varchar(6) DEFAULT NULL");
      // Populate the table using the previous defaults for movies and standard photo extensions.
      // For photos, we preserve distinctions like jpg vs. jpeg vs. JPG vs. JPEG.  For movies,
      // we set as jpg.  This preserves the current file structure.  Note that this does not check
      // if the extension matches the actual file format, which can be incorrect for copies of
      // missing_movie and missing_photo images.  Fixing this would be very resource-intensive.
      ...
      db::build()
        ->update("items")
        ->set("resize_dirty", 0)
        ->set("thumb_extension", "jpg")
        ->where("type", "=", "movie")
        ->execute();
      // Because of the previous inconsistency in how album thumbs are made (copy source thumb vs.
      // generate its own thumb as a jpg), there is no (quick) way to tell what format the existing
      // album thumbs have.  So, let's assign jpg (which maintains the current file structure),
      // mark them all as dirty, then set a site warning.  The rebuild shouldn't take too long
      // since album thumbs are now always generated by copying the source thumb.
      // While we're at it, set resize_dirty to 0 for albums.
      db::build()
        ->update("items")
        ->set("resize_dirty", 0)
        ->set("thumb_extension", "jpg")
      ...
      site_status::warning(
        ...
        "graphics_dirty");
      // Finally, let's set the defaults for the extension conversion of resize and thumb images.
      // We'll use the previous defaults - no conversion for photos, jpg conversion for movies.
      module::set_var("gallery", "convert_photo_resize", "");
      module::set_var("gallery", "convert_photo_thumb", "");
      module::set_var("gallery", "convert_movie_thumb", "jpg");
