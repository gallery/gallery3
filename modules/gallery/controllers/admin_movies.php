...
class Admin_Movies_Controller extends Admin_Controller {
  ...
  public function save() {
    ...
    $form = $this->_get_admin_form();
    if ($form->validate()) {
      module::set_var("gallery", "allow_movie_uploads", $form->settings->allow_uploads->value);
  ...
  private function _print_view($form) {
    list ($ffmpeg_version, $ffmpeg_date) = movie::find_ffmpeg_version();
  ...
  private function _get_admin_form() {
    ...
    $group->dropdown("allow_uploads")
      ->label(t("Allow movie uploads into Gallery"))
      ->options(array("default"=>t("only if FFmpeg is found (default)"),
                      "always"=>t("always"), "never"=>t("never")))
      ->selected(module::get_var("gallery", "allow_movie_uploads", "default"));
    $group->checkbox("rebuild_thumbs")
      ->label(t("Mark all movie thumbnails for rebuild"))
