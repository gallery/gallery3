...
class Gallery_Graphics {
   ...
   *
   *   Graphics::add_rule("gallery", "thumb", "GalleryGraphics::resize",
    ...
    try {
      foreach ($ops as $target => $output_file) {
        ...
        case "album":
          ...
          if (!$cover->thumb_dirty) {
            // Make the album cover from the cover item's thumb.  Run GalleryGraphics::resize with
            // null options and it will figure out if this is a direct copy or conversion to jpg.
            $working_file = $cover->thumb_path();
            GalleryGraphics::resize($working_file, $output_file, null, $item);
          }
          break;
        }
      }
  ...
  private static function _replace_image_with_placeholder($item, $target) {
    if ($item->is_album() && !$item->album_cover_item_id) {
      ...
      $input_path = MODPATH . "gallery/assets/graphics/missing_movie.jpg";
    } else {
      $input_path = MODPATH . "gallery/assets/graphics/missing_photo.jpg";
    }
    ...
    try {
      ...
      GalleryGraphics::resize($input_path, $output_path, $options, null);
    } catch (Exception $e) {
      ...
      SiteStatus::warning(
        t2("One of your photos is out of date. <a %attrs>Click here to fix it</a>",
           ...
           array("attrs" => HTML::mark_clean(sprintf(
             ...
             URL::site("admin/maintenance/start/Hook_GalleryTask::rebuild_dirty_images?csrf=__CSRF__"))))),
        "graphics_dirty");
    }
  }
