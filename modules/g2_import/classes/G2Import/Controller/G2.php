 ...
 */
class G2Import_Controller_G2 extends Controller {
  /**
  ...
  public function action_map() {
    ...
    $id = $this->request->query("g2_itemId");
    $view = $this->request->query("g2_view");
    ...
    if (($path && 0 === strpos($path, "tag/")) || $view == "tags.VirtualAlbum") {
      ...
      if ($tag->loaded()) {
        ...
        Item::set_display_context(
          array("Controller_Tags::get_tag_query",   array($tag)),
          array("Controller_Tags::get_breadcrumbs", array($tag)),
          array());
    ...
    case "thumbnail":
      $this->redirect($item->thumb_url(true), 301);

    case "resize":
      $this->redirect($item->resize_url(true), 301);

    case "file":
    case "full":
      $this->redirect($item->file_url(true), 301);
